...
export function B({ promise, stateB, fallbackCountRef }: Props) {
  const data = use(promise);
  const renderCount = useRef(0);
  renderCount.current += 1;

  return (
    <div className="poc-box" data-component="b">
      <div className="poc-box-header">
        <span className="poc-box-name">B.tsx</span>
        <span key={renderCount.current} className="poc-render-badge">
          render #{renderCount.current}
        </span>
      </div>
      <div className="poc-box-row">
        state_b: <strong>{stateB}</strong>
      </div>
